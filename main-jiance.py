...
def filter_available_urls(urls, max_workers=10):
    """
    过滤可用的URL，使用多线程加速检查
    """
    available_urls = []
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_url = {executor.submit(check_and_return, url): url for url in urls}
        ...
        for future in as_completed(future_to_url):
            ...
            try:
                ...
                if result:
                    available_urls.append(result)
                    logging.info(f"URL可用: {result}")
                else:
                    logging.warning(f"URL不可用: {url}")
            except Exception as e:
                logging.error(f"检查URL时出错: {url}, 错误: {e}")
...
def filter_source_urls(template_file):
    ...
    # 过滤可用的URL
    logging.info("开始检查URL可用性...")
    for category, channel_dict in matched_channels.items():
        for channel_name, urls in channel_dict.items():
            if urls:
                available_urls = filter_available_urls(urls)
                matched_channels[category][channel_name] = available_urls
                logging.info(f"频道 '{channel_name}' 可用URL数量: {len(available_urls)}/{len(urls)}")
...
def sort_and_filter_urls(urls, written_urls):
    ...
    filtered_urls = [
        url for url in
        sorted(urls, key=lambda u: not is_ipv6(u) if config.ip_version_priority == "ipv6" else is_ipv6(u))
...
def write_to_files(f_m3u, f_txt, category, channel_name, index, new_url):
    ...
    f_m3u.write(
        f"#EXTINF:-1 tvg-id=\"{index}\" tvg-name=\"{channel_name}\" tvg-logo=\"{logo_url}\" group-title=\"{category}\",{channel_name}\n")
    f_m3u.write(new_url + "\n")
